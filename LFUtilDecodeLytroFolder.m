...
%                     .ResampMethod : 'fast'(default) or 'triangulation' or 'barycentric' (higher resolution)
...
%                           .noClip : Prevent any clipping in the pipeline. Final data in LF must be multiplied by maxLum to recover original dynamic (default=false).
%                .EarlyWhiteBalance : Perform white balance directly on the RAW data, before demosaicing (default = true).
%                 .CorrectSaturated : Process saturated pixels for better highlights (default = true).
% *Options defined in LFLytroDecodeImage :
%                  .HotPixelCorrect : Performs hot pixel correction using raw black images (default=true).
...
% 22 Aug. 2016 :
%   - new decode option : DoAWB (default=false) : automatic white balance (performed after the camera white balance).
%   - new decode option : noClip (default=false) : prevent clipping operations within the pipeline.
%     The final generated mat file have an additional parameter 'maxVal' to retrieve the original dynamic from the exported uint16 format.
% 20 Dec. 2017 :
%   - new decode option : EarlyWhiteBalance : The white balance (from camera metadata) is applied before demosaicing (applies even if ColourCorrect is not given as optional Task).
%   - new decode option : CorrectSaturated : Correct the saturated areas on the RAW image.
...
DecodeOptions = LFDefaultField('DecodeOptions', 'noClip', false);
DecodeOptions = LFDefaultField( 'DecodeOptions', 'EarlyWhiteBalance', true );
DecodeOptions = LFDefaultField( 'DecodeOptions', 'CorrectSaturated', true );
DecodeOptions = LFDefaultField( 'DecodeOptions', 'CLIMLegacyCols', false ); % CLIM-Legacy Colors : reproduce colors as in version 0.4-CLIM
...
for( iFile = 1:length(FileList) )
    ...
    if( SaveRequired && FileOptions.SaveResult )
        if( isfloat(LF) )
            ...
            LF = uint16( LF .* double(intmax('uint16')) ./ maxLum );
        end
        ...
        save('-v7.3', SaveFname, 'GeneratedByInfo', 'LF', 'maxLum', 'LFMetadata', 'WhiteImageMetadata', 'LensletGridModel', 'DecodeOptions', 'RectOptions');
...
Thumb = squeeze(LF(ceil(end/2),ceil(end/2),:,:,:)); % including weight channel for hist equalize
